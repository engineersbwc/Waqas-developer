...
export const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, onBack }) => {
  ...
  return (
    <div className="fixed inset-0 z-[2000] bg-white overflow-y-auto animate-in slide-in-from-bottom duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)]">
      <nav className="sticky top-0 z-[2100] bg-white/90 backdrop-blur-xl px-6 py-8 border-b border-zinc-100">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <button
            onClick={onBack}
            className="group flex items-center space-x-4 text-zinc-500 hover:text-black transition-all"
          >
            <div className="w-12 h-12 rounded-full border border-zinc-200 flex items-center justify-center group-hover:bg-zinc-50 group-hover:border-black transition-all">
      ...
      <main className="max-w-6xl mx-auto px-6 pt-20 pb-40">
        <Reveal>
          <div className="mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full bg-zinc-100 text-[9px] font-black tracking-[0.3em] uppercase text-zinc-500 mb-6">
              {project.tag}
            </span>
            <h1 className="text-5xl md:text-8xl font-black text-zinc-900 font-lexend tracking-tighter leading-[0.9] mb-10">
              {project.title}
            </h1>
            <p className="text-xl md:text-3xl text-zinc-400 font-medium leading-tight max-w-4xl">
              {project.category}
            </p>
          </div>
        </Reveal>
        ...
        <Reveal className="mb-24">
          <div className="w-full aspect-video rounded-[3rem] overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] border border-zinc-100">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
          <div className="lg:col-span-8 space-y-24">
            {/* Overview / Vision */}
            <section>
              <h2 className="text-3xl md:text-5xl font-black text-zinc-900 font-lexend tracking-tighter mb-8">Project Overview</h2>
          ...
          <div className="lg:col-span-4">
            <div className="bg-zinc-50 rounded-[2.5rem] p-10 sticky top-32 border border-zinc-100">
              <h4 className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.4em] mb-12">Project Profile</h4>

              <div className="space-y-10">
                <div>
                  <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-2">Partner</p>
                  <p className="text-zinc-900 font-bold text-xl">{project.client || 'Confidential'}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-2">Expertise</p>
                  <p className="text-zinc-900 font-bold text-lg leading-tight">{project.services?.join(', ')}</p>
                </div>
                {project.technologies && (
                  <div>
                    <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-2">Tech Stack</p>
                    <div className="flex flex-wrap gap-2">
                      {project.technologies.map((tech, idx) => (
                        <span key={idx} className="px-3 py-1 bg-white border border-zinc-100 rounded-lg text-[10px] font-black text-zinc-800 uppercase tracking-wider">{tech}</span>
                      ))}
                    </div>
                  </div>
                )}
                <div>
                  ...
                  <p className="text-zinc-900 font-bold text-xl">{project.timeline}</p>
                </div>
              </div>
              ...
              {project.url && (
                <a
                  ...
                  className="mt-16 w-full inline-flex items-center justify-center px-8 py-5 bg-black text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-[#f5ba41] hover:text-black transition-all duration-300 shadow-xl"
                >
                  Live Preview
